...
def create_arrangement_suggestion(
    results: dict[str, Any], slices: list[dict[str, Any]]
) -> dict[str, Any]:
    """
    ...
    if len(downbeats) > 0:
        for i in range(len(downbeats) - 1):
            ...
            measure_end = (
                downbeats[i + 1] if i + 1 < len(downbeats) else measure_start + 4.0
            )  # Assume 4/4

            # Find chords in this measure
            measure_chords = [c for c in chords if measure_start <= c["start"] < measure_end]

            # Find slices in this measure
            measure_slices = [s for s in slices if measure_start <= s["start_time"] < measure_end]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Complete music production automation workflow")
    ...
    parser.add_argument("--json", action="store_true", help="Export comprehensive JSON analysis")
