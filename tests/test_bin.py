...
class TestBarTrackerProgram(unittest.TestCase):
    ...
    def test_run(self):
        ...
        run_single(self.bin, sample_file, tmp_result, args=["--beats", sample_beats])
        result = np.loadtxt(tmp_result)
        self.assertTrue(np.allclose(result, self.beats))
        # check 22050 Hz sample rate
        run_single(self.bin, sample_file_22050, tmp_result, args=["--beats", sample_beats])
        result = np.loadtxt(tmp_result)
        self.assertTrue(np.allclose(result, self.beats))
    ...
    def test_batch(self):
        ...
        run_batch(self.bin, [sample_file, sample_beats], args=["--beats_suffix", ".beats"])
    ...
    def test_output_downbeats(self):
        run_single(self.bin, sample_file, tmp_result, args=["--down", "--beats", sample_beats])
        result = np.loadtxt(tmp_result)
        self.assertTrue(np.allclose(result, self.downbeats))

    def test_save_load_activations(self):
        # save RNN bar activations
        run_save(self.bin, sample_file, tmp_act, args=["--beats", sample_beats])
        act = np.load(tmp_act)["activations"]
        self.assertTrue(np.allclose(act, self.act["activations"], rtol=1e-3, equal_nan=True))
...
class TestBeatDetectorProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_blstm.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.beat_detector.txt"))
    ...
    def test_binary(self):
        ...
        if len(result) == 10 and len(self.result) == 9:
            ...
            expected_new = np.vstack([np.array([[0.0, 2.0]]), self.result])
...
class TestBeatTrackerProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_blstm.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.beat_tracker.txt"))
...
class TestCNNChordRecognition(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = [
            ...
            for af in ["sample.cnn_chord_features.npz", "sample2.cnn_chord_features.npz"]
        ]
        self.results = [
            load_chords(pj(DETECTIONS_PATH, df))
            for df in ["sample.cnn_chord_recognition.txt", "sample2.cnn_chord_recognition.txt"]
        ]
    ...
    def test_binary(self):
        for sf, true_act, true_res in zip(
            [sample_file, sample2_file], self.activations, self.results
        ):
            # save activations as binary file
...
class TestComplexFluxProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.complex_flux.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.complex_flux.txt"))
...
class TestCNNOnsetDetectorProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.onsets_cnn.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.cnn_onset_detector.txt"))
...
class TestCRFBeatDetectorProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_blstm.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.crf_beat_detector.txt"))
...
class TestDBNBeatTrackerProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_blstm.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.dbn_beat_tracker.txt"))
...
class TestDBNDownBeatTrackerProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.downbeats_blstm.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.dbn_downbeat_tracker.txt"))
...
class TestDCChordRecognition(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.results = [
            load_chords(pj(DETECTIONS_PATH, df))
            for df in ["sample.dc_chord_recognition.txt", "sample2.dc_chord_recognition.txt"]
        ]
...
class TestKeyRecognitionProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.results = [
            ...
            for df in ["sample.key_recognition.txt", "sample2.key_recognition.txt"]
        ]
...
class TestGMMPatternTrackerProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.gmm_pattern_tracker.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.gmm_pattern_tracker.txt"))
        self.downbeat_result = self.result[self.result[:, 1] == 1][:, 0]
...
class TestLogFiltSpecFluxProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.log_filt_spec_flux.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.log_filt_spec_flux.txt"))
...
class TestMMBeatTrackerProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_blstm_mm.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.mm_beat_tracker.txt"))
...
class TestOnsetDetectorProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.onsets_brnn.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.onset_detector.txt"))
...
class TestOnsetDetectorLLProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.onsets_rnn.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.onset_detector_ll.txt"))
...
class TestPianoTranscriptorProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "stereo_sample.notes_cnn.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "stereo_sample.piano_transcriptor.txt"))
    ...
    def test_mirex(self):
        ...
        self.assertTrue(np.allclose(result[:, 1], [1.56, 3.38, 3.5, 3.48, 3.36, 3.4, 4.16, 4.16]))
        self.assertTrue(
            np.allclose(result[:, 2], [523.25, 87.31, 698.46, 349.23, 261.63, 207.65, 622.25, 98])
        )
...
class TestSpectralOnsetDetectionProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.spectral_flux.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.spectral_flux.txt"))
...
class TestSuperFluxProgram(unittest.TestCase):
    ...
    def setUp(self):
        self.bin = pj(program_path, "SuperFlux")
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.super_flux.npz"))
    ...
    def test_batch(self):
        ...
        run_batch(self.bin, [sample_file, sample_beats, sample2_file], outdir=tmp_dir)
...
class TestSuperFluxNNProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.super_flux_nn.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.super_flux_nn.txt"))
...
class TestTCNBeatTrackerProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_tcn_beats.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.tcn_beat_tracker.txt"))
...
class TestTempoDetectorProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_blstm.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.tempo_detector.txt"))
    ...
    def test_all_tempi(self):
        ...
        self.assertTrue(
            np.allclose(
                result,
                [[176.47, 0.475], [117.65, 0.177], [240.00, 0.154], [68.97, 0.099], [82.19, 0.096]],
            )
        )
...
class TestTCNTempoDetectorProgram(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.activations = Activations(pj(ACTIVATIONS_PATH, "sample.beats_tcn_tempo.npz"))
        self.result = np.loadtxt(pj(DETECTIONS_PATH, "sample.tcn_tempo_detector.txt"))
