...
sample_log_filt_spec = LogarithmicFilteredSpectrogram(sample_spec, num_bands=24, mul=1, add=1)
...
sample_superflux_act = Activations(pj(ACTIVATIONS_PATH, "sample.super_flux.npz"))
...
class TestHighFrequencyContentFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                odf[:6], [8.97001563, 9.36399107, 8.64144536, 8.34977449, 8.21097918, 8.40412515]
            )
        )
...
class TestSpectralDiffFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(odf[:6], [0, 0.55715936, 0.64004618, 0.0810971, 0.295396, 0.16324584])
        )
...
class TestSpectralFluxFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(odf[:6], [0, 3.91207361, 2.91675663, 1.38361311, 2.59582925, 2.16986609])
        )
...
class TestSuperfluxFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(odf[:6], [0, 2.08680153, 0.6411702, 0.38634294, 0.40202433, 0.63349575])
        )
...
class TestComplexFluxFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                odf[:6], [0, 0.476213485, 0.0877621323, 0.0593151376, 0.0654867291, 0.0954693183]
            )
        )
...
class TestModifiedKullbackLeiblerFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(odf[:6], [0, 0.71910584, 0.6664055, 0.68092251, 0.69984031, 0.71744561])
        )
...
class TestPhaseDeviationFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(np.allclose(odf[:6], [0, 0, 0.71957183, 0.91994524, 0.9418999, 0.86083585]))
...
class TestWeightedPhaseDeviationFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(odf[:6], [0, 0, 0.19568817, 0.20483065, 0.17890805, 0.16970603])
        )
...
class TestNormalizesWeightedPhaseDeviationFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(odf[:6], [0, 0, 0.46018526, 0.50193471, 0.42031503, 0.40806249])
        )
...
class TestComplexDomainFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                odf[:6],
                [399.29980469, 585.9564209, 262.08010864, 225.84718323, 196.88954163, 200.32469177],
            )
        )
...
class TestRectifiedComplexDomainFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(odf[:6], [0, 394.165222, 119.79425, 96.70564, 122.52311, 92.61698])
        )
...
class TestSpectralOnsetProcessorClass(unittest.TestCase):
    ...
    def test_processors(self):
        ...
        self.assertIsInstance(proc.processors[2], ShortTimeFourierTransformProcessor)
    ...
    def test_scaling(self):
        ...
        self.assertIsInstance(proc.processors[4], LogarithmicSpectrogramProcessor)
        self.assertEqual(proc.processors[5], spectral_flux)

    def test_filtered_scaling(self):
        # with both filtering and logarithmic scaling
        proc = SpectralOnsetProcessor(filterbank=LogarithmicFilterbank, log=np.log10)
        self.assertIsInstance(proc.processors[4], FilteredSpectrogramProcessor)
        self.assertIsInstance(proc.processors[5], LogarithmicSpectrogramProcessor)
    ...
    def test_process(self):
        ...
        self.assertTrue(
            np.allclose(
                odf[:6], [0.0, 100.90120697, 74.44419861, 40.277565, 57.95736313, 46.15561295]
            )
        )
...
class TestPeakPickingFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        # default values
        onsets = peak_picking(sample_superflux_act, 1.1, pre_max=2, post_max=10, pre_avg=30)
...
class TestOnsetPeakPickingProcessorClass(unittest.TestCase):
    ...
    def test_process_online(self):
        ...
        onsets_2 = np.hstack(
            [self.online_processor(np.atleast_1d(f), reset=False) for f in sample_rnn_act]
        )
