...
def _compare_labels(test_case, labels, reference_labels):
    test_case.assertTrue(np.allclose(labels["start"], reference_labels["start"]))
...
class TestParseChords(unittest.TestCase):
    ...
    def test_read_chord_annotations(self):
        chords = load_chords(pj(DETECTIONS_PATH, "sample2.dc_chord_recognition.txt"))
        _compare_labels(
            self,
            chords,
            np.array(
                [(0.0, 1.6, "F:maj"), (1.6, 2.5, "A:maj"), (2.5, 4.1, "D:maj")], dtype=SEGMENT_DTYPE
            ),
        )

        chords = load_chords(pj(DETECTIONS_PATH, "sample.dc_chord_recognition.txt"))
        _compare_labels(self, chords, np.array([(0.0, 2.9, "G#:maj")], dtype=SEGMENT_DTYPE))
...
class TestMajMinTargetsToChordLabelsFunction(unittest.TestCase):
    ...
    def test_frame_join(self):
        ...
        target_labels = np.array(
            [(0.0, 0.2, "A:maj"), (0.2, 0.6, "C#:maj"), (0.6, 0.7, "N"), (0.7, 0.9, "F:maj")],
            dtype=SEGMENT_DTYPE,
        )
        labels = majmin_targets_to_chord_labels(targets, fps)
        _compare_labels(self, labels, target_labels)
...
class TestDeepChromaChordRecognitionProcessorClass(unittest.TestCase):
    ...
    def test_process(self):
        for activation, true_labels in zip(sample_deep_chroma_acts, sample_deep_chroma_labels):
            labels = self.processor(activation)
            _compare_labels(self, labels, true_labels)
