...
class TestOnsetEvaluationFunction(unittest.TestCase):
    ...
    def test_results(self):
        ...
        self.assertTrue(np.allclose(errors, [-0.00000001, 0.00999999, 0.01, -0.01, 0]))
        # window = 0.01
        tp, fp, tn, fn, errors = onset_evaluation(DETECTIONS, ANNOTATIONS, window=0.01)
        self.assertTrue(np.allclose(tp, [0.999999, 1.029999, 2.01, 2.02, 2.5]))
        self.assertTrue(np.allclose(fp, [1.45, 3.025000001]))
        self.assertTrue(np.allclose(tn, []))
        self.assertTrue(np.allclose(fn, [1.5, 2.05, 3.0]))
        self.assertTrue(np.allclose(errors, [-0.00000001, 0.00999999, 0.01, -0.01, 0]))
        # window = 0.04
        tp, fp, tn, fn, errors = onset_evaluation(DETECTIONS, ANNOTATIONS, window=0.04)
        self.assertTrue(np.allclose(tp, [0.999999, 1.029999, 2.01, 2.02, 2.5, 3.025000001]))
        self.assertTrue(np.allclose(fp, [1.45]))
        self.assertTrue(np.allclose(tn, []))
        self.assertTrue(np.allclose(fn, [1.5, 2.05]))
        self.assertTrue(np.allclose(errors, [-0.00000001, 0.00999999, 0.01, -0.01, 0, 0.025]))
...
class TestOnsetEvaluationClass(unittest.TestCase):
    ...
    def test_results(self):
        ...
        # p = correct / retrieved
        self.assertEqual(e.precision, 5.0 / 7.0)
        # r = correct / relevant
        self.assertEqual(e.recall, 5.0 / 8.0)
        # f = 2 * P * R / (P + R)
        f = 2 * (5.0 / 7.0) * (5.0 / 8.0) / ((5.0 / 7.0) + (5.0 / 8.0))
        self.assertEqual(e.fmeasure, f)
        # acc = (TP + TN) / (TP + FP + TN + FN)
        self.assertEqual(e.accuracy, (5.0 + 0) / (5 + 2 + 0 + 3))
        # errors
        # det 0.99999999, 1.02999999, 1.45, 2.01, 2.02,       2.5, 3.030000001
        # tar 1,          1.02,       1.5,  2.0,  2.03, 2.05, 2.5, 3
        errors = [
            0.99999999 - 1,
            1.02999999 - 1.02,  # 1.45 - 1.5,
            2.01 - 2,
            2.02 - 2.03,
            2.5 - 2.5,
        ]  # , 3.030000001 - 3
        self.assertTrue(np.allclose(e.errors, errors))
        mean = np.mean([0.99999999 - 1, 1.02999999 - 1.02, 2.01 - 2, 2.02 - 2.03, 2.5 - 2.5])
        self.assertEqual(e.mean_error, mean)
        std = np.std([0.99999999 - 1, 1.02999999 - 1.02, 2.01 - 2, 2.02 - 2.03, 2.5 - 2.5])
...
class TestOnsetMeanEvaluationClass(unittest.TestCase):
    ...
    def test_results(self):
        ...
        self.assertTrue(np.allclose(e.num_tp, np.mean([e_.num_tp for e_ in [e1, e2, e3]])))
        self.assertTrue(np.allclose(e.num_fp, np.mean([e_.num_fp for e_ in [e1, e2, e3]])))
        self.assertTrue(np.allclose(e.num_tn, np.mean([e_.num_tn for e_ in [e1, e2, e3]])))
        self.assertTrue(np.allclose(e.num_fn, np.mean([e_.num_fn for e_ in [e1, e2, e3]])))
        self.assertTrue(np.allclose(e.precision, np.mean([e_.precision for e_ in [e1, e2, e3]])))
        self.assertTrue(np.allclose(e.recall, np.mean([e_.recall for e_ in [e1, e2, e3]])))
        self.assertTrue(np.allclose(e.fmeasure, np.mean([e_.fmeasure for e_ in [e1, e2, e3]])))
        self.assertTrue(np.allclose(e.accuracy, np.mean([e_.accuracy for e_ in [e1, e2, e3]])))
        # errors is just a concatenation of all errors
        # (inherited from SumOnsetEvaluation)
        self.assertTrue(np.allclose(e.errors, np.concatenate([e_.errors for e_ in [e2, e3]])))
        # mean and std errors are those of e2 and e3, since those of e1 are NaN
        self.assertEqual(e.mean_error, np.mean([e_.mean_error for e_ in [e2, e3]]))
...
class TestAddParserFunction(unittest.TestCase):
    ...
    def test_args(self):
        args = self.parser.parse_args(["onsets", ANNOTATIONS_PATH, DETECTIONS_PATH])
