...
class TestLoadWaveFileFunction(unittest.TestCase):
    ...
    def test_values(self):
        # test wave loader
        signal, sample_rate = load_wave_file(sample_file)
        self.assertTrue(np.allclose(signal[:5], [-2494, -2510, -2484, -2678, -2833]))
        ...
        self.assertTrue(np.allclose(signal[:4], [[33, 38], [35, 36], [29, 34], [36, 31]]))
    ...
    def test_start_stop(self):
        # test wave loader
        signal, sample_rate = load_wave_file(sample_file, start=1.0 / 44100, stop=5.0 / 44100)
    ...
    def test_downmix(self):
        # test wave loader
        signal, sample_rate = load_wave_file(stereo_sample_file, num_channels=1)
    ...
    def test_upmix(self):
        ...
        self.assertTrue(
            np.allclose(
                signal[:5],
                [[-2494, -2494], [-2510, -2510], [-2484, -2484], [-2678, -2678], [-2833, -2833]],
            )
        )
        self.assertTrue(len(signal) == 123481)
        self.assertTrue(sample_rate == 44100)
...
class TestLoadAudioFileFunction(unittest.TestCase):
    ...
    def test_start_stop(self):
        # test wave loader
        signal, sample_rate = load_audio_file(sample_file, start=1.0 / 44100, stop=5.0 / 44100)
        self.assertTrue(np.allclose(signal, [-2510, -2484, -2678, -2833]))
        self.assertTrue(len(signal) == 4)
        self.assertTrue(sample_rate == 44100)
        # test ffmpeg loader
        signal, sample_rate = load_audio_file(
            stereo_sample_file, start=1.0 / 44100, stop=4.0 / 44100
        )
    ...
    def test_downmix(self):
        # test wave loader
        signal, sample_rate = load_audio_file(stereo_sample_file, num_channels=1)
        self.assertTrue(np.allclose(signal[:5], [35, 35, 31, 33, 33]))
        self.assertTrue(len(signal) == 182919)
        self.assertTrue(sample_rate == 44100)
        self.assertTrue(signal.shape == (182919,))
        ...
        self.assertTrue(np.allclose(signal_stereo.mean(axis=1), signal_stereo[:, 0]))
        # check clipping
    ...
    def test_resample(self):
        ...
        signal, sample_rate = load_audio_file(stereo_sample_file, sample_rate=22050)
        self.assertTrue(sample_rate == 22050)
        # avconv does round differently, thus allow atol=1
        # result: [[33, 38], [33, 33], [36, 31], [35, 35], [32, 35]]
        self.assertTrue(
            np.allclose(signal[:5], [[34, 38], [32, 33], [37, 31], [35, 35], [32, 34]], atol=1)
        )
        # also downmix
        signal, sample_rate = load_audio_file(stereo_sample_file, sample_rate=22050, num_channels=1)
    ...
    def test_choose_channel(self):
        signal, sample_rate = load_audio_file(
            flac_file, sample_rate=22050, num_channels=1, channel=0
        )
        # avconv results in a different length of 91450 samples
        self.assertTrue(np.allclose(len(signal), 91460, atol=10))
    ...
    def test_replaygain_track(self):
        original = load_signal(flac_file)
        data, sample_rate = load_audio_file(rg_flac_file, replaygain_mode="track")
        signal = Signal(data)
    ...
    def test_replaygain_preamp(self):
        ...
        pre_rg_sig = load_signal(loud_rg_flac_file, replaygain_mode="track", replaygain_preamp=+6.0)
...
class TestDecodeToDisk(unittest.TestCase):
    ...
    def test_write_with_replaygain(self):
        ...
        outfile_rg = decode_to_disk(loud_rg_flac_file, tmp_suffix=".raw", replaygain_mode="track")
