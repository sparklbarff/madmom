...
from madmom.audio.filters import BarkFilterbank, MelFilterbank
...
class TestSpectrogramClass(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                result[0, :8],
                [3.15249, 4.00272, 5.66156, 6.30141, 6.02199, 10.84909, 17.83130, 19.44511],
            )
        )
        self.assertTrue(
            np.allclose(
...
class TestFilteredSpectrogramClass(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                result[0, :8],
                [5.661564, 6.30141, 6.02199, 10.84909, 17.8313, 19.44511, 17.56456, 21.859523],
            )
        )
        self.assertTrue(
            np.allclose(
                result[0, -8:],
                [0.123125, 0.119462, 0.137849, 0.1269156, 0.110888, 0.083526, 0.05426, 0.064614],
            )
        )
        # attributes
        self.assertTrue(result.shape == (281, 81))
        self.assertTrue(np.allclose(result.bin_frequencies, LOG_FILTERBANK_CENTER_FREQS))
        # properties
        self.assertTrue(result.num_bins == 81)
        self.assertTrue(result.num_frames == 281)
        # with given filterbank
        result = FilteredSpectrogram(sample_file, filterbank=result.filterbank)
        # attributes
        self.assertTrue(result.shape == (281, 81))
        self.assertTrue(np.allclose(result.bin_frequencies, LOG_FILTERBANK_CENTER_FREQS))
        # properties
    ...
    def test_filterbanks(self):
        # with Mel filterbank
        result = FilteredSpectrogram(sample_file, filterbank=MelFilterbank, num_bands=40)
        self.assertTrue(
            np.allclose(
                result[0, :6],
                [8.42887115, 17.98174477, 19.50165367, 6.48194313, 2.96991181, 4.06280804],
            )
        )
        self.assertTrue(result.shape == (281, 40))
        # with Bark filterbank
        result = FilteredSpectrogram(sample_file, filterbank=BarkFilterbank, num_bands="normal")
        self.assertTrue(
            np.allclose(
                result[0, :6],
                [16.42251968, 17.36715126, 2.81979132, 4.27050114, 3.08699131, 1.50553513],
            )
        )
...
class TestFilteredSpectrogramProcessorClass(unittest.TestCase):
    ...
    def test_types(self):
        ...
        self.assertTrue(issubclass(self.processor.filterbank, LogarithmicFilterbank))
    ...
    def test_other_values(self):
        processor = FilteredSpectrogramProcessor(num_bands=6, fmin=300, fmax=10000)
...
class TestLogarithmicSpectrogramClass(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                result[0, :8],
                [0.618309, 0.699206, 0.823576, 0.86341, 0.84646, 1.073685, 1.27488, 1.310589],
            )
        )
        self.assertTrue(
            np.allclose(
        ...
        self.assertTrue(result.num_bins == 1024)
        # test other values
        result = LogarithmicSpectrogram(sample_file, mul=2, add=2)
...
class TestLogarithmicFilteredSpectrogramClass(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                result[0, :8],
                [0.8235762, 0.863407, 0.8464602, 1.073685, 1.27488, 1.3105896, 1.2686847, 1.359067],
            )
        )
        self.assertTrue(
            np.allclose(
...
class TestLogarithmicFilteredSpectrogramProcessorClass(unittest.TestCase):
    ...
    def test_types(self):
        ...
        self.assertIsInstance(self.processor, LogarithmicFilteredSpectrogramProcessor)
...
class TestSpectrogramDifferenceProcessorClass(unittest.TestCase):
    ...
    def test_process(self):
        ...
        result_2 = np.vstack(
            [self.processor.process(np.atleast_2d(frame), reset=False) for frame in sample_spec]
        )
...
class TestSuperFluxProcessorClass(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                result[1, :8], [0.11168772, 0.12317812, 0, 0, 0.03797626, 0.18899226, 0, 0.0903399]
            )
        )
        self.assertTrue(
            np.allclose(
                result[1, -8:],
                [0, 0, 0.01419619, 0, 0.02666602, 0.04325962, 0.10899737, 0.06546581],
            )
        )
        self.assertIsInstance(result, SpectrogramDifference)
...
class TestMultiBandSpectrogramClass(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(result.bin_frequencies, [86.1328125, 581.39648438, 8979.34570312])
        )
...
class TestMultiBandSpectrogramProcessorClass(unittest.TestCase):
    ...
    def test_values(self):
        self.assertTrue(np.allclose(self.processor.crossover_frequencies, [200, 1000]))
        self.assertTrue(self.processor.fmin == 30)
        self.assertTrue(self.processor.fmax == 17000)
        self.assertTrue(self.processor.norm_filters is True)
        self.assertTrue(self.processor.unique_filters is True)
    ...
    def test_process(self):
        ...
        self.assertTrue(np.allclose(result.bin_frequencies, [86.1328, 581.3965, 8979.3457]))
        # properties
        self.assertTrue(result.num_bins == 3)
        self.assertTrue(result.num_frames == 281)
        # test 2 bands
        ...
        self.assertTrue(
            np.allclose(
                result[:3],
                [[9.37507915, 0.23498698], [10.4683371, 0.26268598], [10.8684139, 0.24078195]],
            )
        )
        self.assertIsInstance(result, MultiBandSpectrogram)
        self.assertTrue(result.shape == (281, 2))
        self.assertTrue(np.allclose(result.crossover_frequencies, [500]))
        self.assertTrue(np.allclose(result.bin_frequencies, [236.865, 8720.947]))
        self.assertTrue(np.allclose(np.max(result.filterbank, axis=0), [0.04545455, 0.00130548]))
        # properties
        ...
        self.assertTrue(
            np.allclose(
                result[:3],
                [[206.25172424, 180], [230.30342102, 201.21743774], [239.10510254, 184.43896484]],
            )
        )
        self.assertIsInstance(result, MultiBandSpectrogram)
        self.assertTrue(result.shape == (281, 2))
...
class TestSemitoneBandpassSpectrogramClass(unittest.TestCase):
    ...
    def test_process(self):
        ...
        self.assertTrue(
            np.allclose(
                self.sbs_50[120:122, 50:55],
                [
                    [0.00056659, 0.00274373, 0.00037994, 0.00031497, 0.0063823],
                    [0.00032294, 0.00285728, 0.00023723, 0.00010553, 0.0069074],
                ],
                atol=1e-04,
            )
        )
        ...
        sbs_10 = [
            [0.01951193, 0.01638364, 0.00384092, 0.00732366, 0.10310112],
            [0.14484727, 0.032042, 0.00719009, 0.02043642, 0.06407038],
        ]
        self.assertTrue(np.allclose(self.sbs_10[10:12, 50:55], sbs_10, atol=1e-04))
        # test computing SemitoneBandpassSpectrogram from signal
        self.assertTrue(self.sbs_10_from_signal.shape == (29, 88))
        self.assertTrue(np.allclose(self.sbs_10_from_signal[10:12, 50:55], sbs_10, atol=1e-04))
        ...
        self.assertTrue(np.allclose(self.sbs_22050[108:110, :], tar, atol=1e-04))
        # check end of signal
        tar = [
            9.44913489e-06,
            2.15330783e-05,
    ...
    def test_compare_with_matlab_toolbox(self):
        ...
        self.assertTrue(
            np.allclose(
                self.sbs_10[10:12, 50:55],
                [
                    [0.01951726, 0.01638535, 0.00384128, 0.00732471, 0.10306561],
                    [0.14487972, 0.03204085, 0.00718818, 0.02043327, 0.06404668],
                ],
                rtol=1e-03,
            )
        )
