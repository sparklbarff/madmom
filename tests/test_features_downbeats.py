...
sample_downbeat_act = Activations(pj(ACTIVATIONS_PATH, "sample.downbeats_blstm.npz"))
sample_pattern_features = Activations(pj(ACTIVATIONS_PATH, "sample.gmm_pattern_tracker.npz"))
...
class TestRNNDownBeatProcessorClass(unittest.TestCase):
    ...
    def test_process(self):
        ...
        self.assertTrue(np.allclose(downbeat_act, sample_downbeat_act, atol=1e-5))


class TestDBNDownBeatTrackingProcessorClass(unittest.TestCase):
    def setUp(self):
        self.processor = DBNDownBeatTrackingProcessor([3, 4], fps=sample_downbeat_act.fps)
    ...
    def test_types(self):
        ...
        self.assertIsInstance(self.processor.hmms[0].transition_model, BarTransitionModel)
        self.assertIsInstance(
            self.processor.hmms[0].observation_model, RNNDownBeatTrackingObservationModel
        )
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                path[:13], [7682, 7683, 7684, 7685, 7686, 7687, 7688, 7689, 217, 218, 219, 220, 221]
            )
        )
    ...
    def test_process(self):
        ...
        self.assertTrue(
            np.allclose(downbeats, [[0, 1], [0.49, 2], [0.99, 3], [1.49, 4], [1.99, 1]])
        )
        ...
        expected_threshold = correct[
            1:-1
        ]  # Should be 8 beats: [0.08, 0.43, 0.77, 1.11, 1.45, 1.79, 2.13, 2.47]
...
class TestPatternTrackingProcessorClass(unittest.TestCase):
    ...
    def test_types(self):
        ...
        self.assertIsInstance(self.processor.om, GMMPatternTrackingObservationModel)
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                path[:12], [5573, 5574, 5575, 5576, 6757, 6758, 6759, 6760, 6761, 6762, 6763, 6764]
            )
        )
        self.assertTrue(np.allclose(prob, -468.8014))
        patterns = self.processor.st.state_patterns[path]
        self.assertTrue(np.allclose(patterns, np.ones(len(sample_pattern_features))))
        positions = self.processor.st.state_positions[path]
        self.assertTrue(
            np.allclose(
                positions[:6], [1.76470588, 1.82352944, 1.88235296, 1.94117648, 2, 2.0588236]
            )
        )
...
class TestLoadBeatsProcessorClass(unittest.TestCase):
    ...
    def test_batch(self):
        proc = LoadBeatsProcessor(None, files=DETECTION_FILES, beats_suffix=".dbn_beat_tracker.txt")
...
class TestSyncronizeFeaturesProcessorClass(unittest.TestCase):
    ...
    def setUp(self):
        self.processor = SyncronizeFeaturesProcessor(beat_subdivisions=2, fps=100)
    ...
    def test_corner_cases(self):
        ...
        self.assertTrue(np.allclose(feat_sync, [[[11.0], [33.5]]]))
...
class TestRNNBarProcessorClass(unittest.TestCase):
    ...
    def test_process(self):
        ...
        self.assertTrue(np.allclose(act, sample_bar_act, rtol=1e-3, equal_nan=True))
...
class TestDBNBarTrackingProcessorClass(unittest.TestCase):
    ...
    def test_process(self):
        ...
        self.assertTrue(
            np.allclose(beats, [[0.0913, 1.0], [0.7997, 2.0], [1.4806, 3.0], [2.1478, 1.0]])
        )
