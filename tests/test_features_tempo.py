...
from madmom.io import load_tempo, write_tempo
...
COMB_TEMPI = np.array(
    [[176.470, 0.475], [117.647, 0.177], [240.0, 0.154], [68.966, 0.099], [82.192, 0.096]]
)
COMB_TEMPI_ONLINE = [
    [176.470588, 0.289414003],
    [115.384615, 0.124638601],
    [230.769231, 0.0918372569],
    [84.5070423, 0.0903815502],
    [75.0000000, 0.0713704506],
    [53.5714286, 0.0701783497],
    [65.9340659, 0.0696296514],
    [49.1803279, 0.0676349815],
    [61.2244898, 0.0646209647],
    [40.8163265, 0.0602941909],
]
ACF_TEMPI = np.array(
    [
        [176.470, 0.246],
...
ACF_TEMPI_ONLINE = [
    [176.470588, 0.253116038],
    [88.2352941, 0.231203195],
    [58.8235294, 0.187827698],
    [43.7956204, 0.139373027],
    [115.384615, 0.0749783568],
    [69.7674419, 0.0599632291],
    [50.4201681, 0.0535384559],
]
DBN_TEMPI = np.array([[176.470, 1]])
DBN_TEMPI_ONLINE = [
    [176.470588, 0.580877380],
    [86.9565217, 0.244729904],
    [74.0740741, 0.127887992],
    [40.8163265, 0.0232523621],
    [250.000000, 0.0232523621],
]
TCN_TEMPI = np.array(
    [[87, 0.62103526], [175, 0.2131467], [58, 0.1607556], [41, 0.00323059], [115, 0.0008726]]
)
...
class TestIntervalHistogramAcfFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                hist[0][:6],
                [0.10034907, 0.10061631, 0.11078519, 0.13461014, 0.17694432, 0.24372872],
            )
        )
...
class TestIntervalHistogramCombFunction(unittest.TestCase):
    ...
    def setUp(self):
        self.hist_0_6 = np.array(
            [3.16024358, 2.00690881, 2.52592621, 2.00221429, 1.73527979, 1.47528936]
        )
...
class TestSmoothHistogramFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                hist[0][:6],
                [3.32079628, 2.46180239, 2.84665606, 2.34311077, 2.01348008, 1.73241048],
            )
        )
...
class TestDetectTempoFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(np.allclose(result[:6, 0], 6000.0 / self.bpm_result[:, 0], atol=0.1))
        self.assertTrue(
            np.allclose(result[:6, 1], [0.1689, 0.0635, 0.0508, 0.0406, 0.0402, 0.04], atol=1e-4)
        )
    ...
    def test_interpolation(self):
        ...
        self.assertTrue(
            np.allclose(
                result[:5],
                [[176.16, 0.111], [116.75, 0.044], [250.0, 0.033], [82.37, 0.027], [230.33, 0.027]],
                atol=0.1,
            )
        )
...
class TestTempoEstimationProcessorClass(unittest.TestCase):
    ...
    def test_types(self):
        ...
        self.assertIsInstance(self.processor.histogram_processor, TempoHistogramProcessor)
    ...
    def test_process_online(self):
        ...
        tempi = [self.online_processor(np.atleast_1d(a), reset=False) for a in act]
        ...
        self.assertTrue(
            np.allclose(
                tempi[-1][:3],
                [[176.470588, 0.31322337], [85.7142857, 0.11437361], [115.384615, 0.10919612]],
            )
        )
...
class TestCombFilterTempoHistogramProcessorClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.online_processor = CombFilterTempoHistogramProcessor(fps=fps, online=True)
    ...
    def test_tempo(self):
        tempo_processor = TempoEstimationProcessor(histogram_processor=self.processor, fps=fps)
    ...
    def test_process_online(self):
        ...
        self.online_processor.reset()
        result = [self.online_processor(np.atleast_1d(a), reset=False) for a in act]
...
class TestACFTempoHistogramProcessorClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.online_processor = ACFTempoHistogramProcessor(fps=fps, online=True)
    ...
    def test_tempo_online(self):
        ...
        self.assertTrue(
            np.allclose(
                tempi[-1][:3],
                [[176.4705882, 0.2414368], [86.95652174, 0.2248635], [58.25242718, 0.1878183]],
            )
        )
...
class TestDBNTempoHistogramProcessorClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.online_processor = DBNTempoHistogramProcessor(fps=fps, online=True)
    ...
    def test_tempo_online(self):
        ...
        self.assertTrue(
            np.allclose(
                tempi[-1][:3],
                [[176.4705882, 0.472499032], [84.5070423, 0.432130320], [74.0740741, 0.0699384753]],
            )
        )
...
class TestTCNTempoHistogramProcessorClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.act = Activations(pj(ACTIVATIONS_PATH, "sample.beats_tcn_tempo.npz"))
    ...
    def test_tempo(self):
        ...
        self.assertTrue(
            np.allclose(
                tempi[:, 0], [87, 174, 58, 41, 115, 132, 100, 32, 197, 246, 228, 23, 214, 14]
            )
        )
    ...
    def test_tempo_hist_smooth(self):
        tempo_processor = TempoEstimationProcessor(
            histogram_processor=self.processor, act_smooth=None, hist_smooth=15
        )
        tempi = tempo_processor(self.act)
...
class TestWriteTempoFunction(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(np.allclose(result, [[176.47, 1]], atol=1e-4, equal_nan=True))
        # only one tempo given (<68 bpm)
        write_tempo(COMB_TEMPI[3] / 2, self.out_file)
        result = load_tempo(self.out_file)
        self.assertTrue(np.allclose(result, [[34.48, 1]], atol=1e-4, equal_nan=True))
        # multiple tempi given
        write_tempo(COMB_TEMPI, self.out_file)
        result = load_tempo(self.out_file)
        self.assertTrue(np.allclose(result, [[176.47, 0.73], [117.65, 0.27]], atol=1e-4))

    def test_values_mirex(self):
        # multiple tempi given
        write_tempo(COMB_TEMPI, self.out_file, mirex=True)
        result = load_tempo(self.out_file)
        self.assertTrue(np.allclose(result, [[117.65, 0.27], [176.47, 0.73]], atol=1e-4))
