...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_key_prediction_to_label_function(self):
        ...
        self.assertEqual(key_prediction_to_label(sample_key_act[0]), "Ab major")
        self.assertEqual(key_prediction_to_label(np.roll(sample_key_act[0], 1)), "A minor")
        self.assertEqual(key_prediction_to_label(np.roll(sample_key_act[0], -3)), "F major")

        self.assertEqual(key_prediction_to_label(sample2_key_act), "A minor")
        self.assertEqual(key_prediction_to_label(sample2_key_act[0]), "A minor")
        self.assertEqual(key_prediction_to_label(np.roll(sample2_key_act[0], 1)), "Bb minor")
        self.assertEqual(key_prediction_to_label(np.roll(sample2_key_act[0], -3)), "F# major")
