...
from . import ACTIVATIONS_PATH, ANNOTATIONS_PATH, AUDIO_PATH, DATA_PATH, DETECTIONS_PATH
from .test_evaluation_notes import ANNOTATIONS as NOTES

FILE_LIST = [pj(DATA_PATH, "README"), pj(DATA_PATH, "commented_txt"), pj(DATA_PATH, "events.txt")]
...
ACTIVATION_FILES = [
    pj(ACTIVATIONS_PATH, "sample.bar_tracker.npz"),
...
ANNOTATION_FILES = [
    pj(ANNOTATIONS_PATH, "dummy.chords"),
...
DETECTION_FILES = [
    pj(DETECTIONS_PATH, "dummy.chords.txt"),
...
class TestFilterFilesFunction(unittest.TestCase):
    ...
    def test_single_file(self):
        ...
        # suffix list
        result = filter_files(pj(DATA_PATH, "events.txt"), suffix=[".txt", None])
    ...
    def test_file_list(self):
        ...
        self.assertEqual(result, [pj(DATA_PATH, "commented_txt"), pj(DATA_PATH, "events.txt")])
        # suffix list
        result = filter_files(FILE_LIST, suffix=[".txt"])
        self.assertEqual(result, [pj(DATA_PATH, "events.txt")])
...
class TestSearchPathFunction(unittest.TestCase):
    ...
    def test_recursion(self):
        ...
        all_files = FILE_LIST + AUDIO_FILES + ANNOTATION_FILES + DETECTION_FILES + ACTIVATION_FILES
...
class TestSearchFilesFunction(unittest.TestCase):
    ...
    def test_path(self):
        ...
        file_list = [pj(DATA_PATH, "commented_txt"), pj(DATA_PATH, "events.txt")]
        self.assertEqual(result, sorted(file_list))
        # another suffix
        result = search_files(DATA_PATH, suffix=".txt")
...
class TestMatchFileFunction(unittest.TestCase):
    ...
    def test_match_other_suffix(self):
        ...
        self.assertEqual(result, [])
        result = match_file("file.txt", match_list, match_suffix="other", match_exactly=False)
    ...
    def test_match_dot_other_suffix(self):
        ...
        result = match_file("file.txt", match_list, match_suffix=".other")
        self.assertEqual(result, ["/path/file.txt.other"])
        result = match_file("txt", match_list, match_suffix=".other", match_exactly=False)
        self.assertEqual(result, ["/path/file.txt.other"])
    ...
    def test_match_any_suffix(self):
        ...
        self.assertEqual(result, ["file.txt", "/path/file.txt"])
        result = match_file("file.txt", match_list, match_suffix="*", match_exactly=False)
...
class TestCombineEventsFunction(unittest.TestCase):
    ...
    def test_combine_mean(self):
        # EVENTS =           [1, 1.02, 1.5, 2.0, 2.03, 2.05, 2.5, 3]
        comb = combine_events(EVENTS, 0.0)
    ...
    def test_combine_left(self):
        # EVENTS =           [1, 1.02, 1.5, 2.0, 2.03, 2.05, 2.5, 3]
        comb = combine_events(EVENTS, 0.0, "left")
        correct = np.asarray([1, 1.02, 1.5, 2.0, 2.03, 2.05, 2.5, 3])
        self.assertTrue(np.allclose(comb, correct))
    ...
    def test_combine_right(self):
        # EVENTS =           [1, 1.02, 1.5, 2.0, 2.03, 2.05, 2.5, 3]
        comb = combine_events(EVENTS, 0.0, "right")
        correct = np.asarray([1, 1.02, 1.5, 2.0, 2.03, 2.05, 2.5, 3])
        self.assertTrue(np.allclose(comb, correct))
...
class TestQuantizeNotesFunction(unittest.TestCase):
    ...
    def test_fps(self):
        ...
        correct = np.arange(
            np.round(NOTES[0, 0] * fps), np.round((NOTES[0, 0] + NOTES[0, 2]) * fps) + 1
        )
        ...
        idx = np.nonzero(quantized)
        correct = np.arange(
            np.round(NOTES[1, 0] * fps), np.round((NOTES[1, 0] + NOTES[1, 2]) * fps) + 1
        )
...
class TestSegmentAxisFunction(unittest.TestCase):
    ...
    def test_values(self):
        result = segment_axis(np.arange(10), 4, 2)
        self.assertTrue(
            np.allclose(result, [[0, 1, 2, 3], [2, 3, 4, 5], [4, 5, 6, 7], [6, 7, 8, 9]])
        )
        result = segment_axis(np.arange(10), 4, 3, end="pad")
        self.assertTrue(np.allclose(result, [[0, 1, 2, 3], [3, 4, 5, 6], [6, 7, 8, 9]]))
        result = segment_axis(np.arange(11), 4, 3, end="pad")
        self.assertTrue(
            np.allclose(result, [[0, 1, 2, 3], [3, 4, 5, 6], [6, 7, 8, 9], [9, 10, 0, 0]])
        )
        result = segment_axis(np.arange(11), 4, 3, end="pad", end_value=1)
        self.assertTrue(
            np.allclose(result, [[0, 1, 2, 3], [3, 4, 5, 6], [6, 7, 8, 9], [9, 10, 1, 1]])
        )
        result = segment_axis(np.arange(11), 4, 3, end="wrap")
        self.assertTrue(
            np.allclose(result, [[0, 1, 2, 3], [3, 4, 5, 6], [6, 7, 8, 9], [9, 10, 0, 1]])
        )
