...
class TestEvaluateScript(unittest.TestCase):
    ...
    def test_onsets(self):
        ...
        self.assertTrue(np.allclose(sum_res, [14, 2, 0, 1, 15, 0.875, 0.933, 0.903, 0.824]))
    ...
    def test_beats(self):
        ...
        self.assertTrue(np.allclose(res, [0.667, 0.5, 0.639, 1, 0, 0, 0.875, 0.875, 3.322, 3.322]))

    def test_downbeats(self):
        with self.assertRaises(BeatIntervalError):
            run_script("beats", det_suffix=".dbn_downbeat_tracker.txt", args=["--down"])
    ...
    def test_chords(self):
        ...
        self.assertTrue(
            np.allclose(weighted, [0.897, 0.641, 0.462, 0.462, 0.282, 0.590, 0.590, 0.923])
        )
    ...
    def test_notes(self):
        res = run_script("notes", det_suffix=".piano_transcriptor.txt", args=["-w", "0.05"])
        # second line contains the summed results
        sum_res = np.fromiter(res[1].split(",")[1:], dtype=float)
        self.assertTrue(np.allclose(sum_res, [8, 0, 0, 0, 8, 1, 1, 1, 1]))
        # third line contains the mean results
        mean_res = np.fromiter(res[2].split(",")[1:], dtype=float)
        self.assertTrue(np.allclose(mean_res, sum_res))

    def test_tempo(self):
        res = run_script("tempo", det_suffix=".tempo_detector.txt", args=["-a", ".tempo"])
        # second line contains the results
        res = np.fromiter(res[1].split(",")[1:], dtype=float)
        self.assertTrue(np.allclose(res, [0.3, 1, 0, 0, 1]))
