...
sample_tcn_act = Activations(pj(ACTIVATIONS_PATH, "sample.beats_tcn_beats.npz"))
...
class TestRNNBeatProcessorClass(unittest.TestCase):
    ...
    def test_process_lstm(self):
        ...
        self.processor = RNNBeatProcessor(online=True, num_frames=1, origin="future")
...
class TestBeatTrackingProcessorClass(unittest.TestCase):
    ...
    def test_process(self):
        ...
        self.assertTrue(np.allclose(beats, [0.11, 0.45, 0.79, 1.13, 1.47, 1.81, 2.15, 2.49]))
...
class TestDBNBeatTrackingProcessorClass(unittest.TestCase):
    ...
    def test_values(self):
        ...
        self.assertTrue(
            np.allclose(
                path[:15],
                [207, 208, 209, 210, 211, 212, 213, 214, 215, 216, 183, 184, 185, 186, 187],
            )
        )
    ...
    def test_process(self):
        beats = self.processor(sample_blstm_act)
        self.assertTrue(np.allclose(beats, [0.1, 0.45, 0.8, 1.12, 1.48, 1.8, 2.15, 2.49]))
        ...
        beats = self.processor(sample_blstm_act)
        self.assertTrue(np.allclose(beats, [0.1, 0.44, 0.78, 1.12, 1.46, 1.8, 2.14, 2.48]))
        # set the threshold
        self.processor.threshold = 0.25
        beats = self.processor(sample_blstm_act)
        self.assertTrue(np.allclose(beats, [0.09, 0.43, 0.77, 1.11, 1.45, 1.79, 2.13]))
    ...
    def test_process_forward(self):
        ...
        processor.reset()
        beats = [
            processor.process_forward(np.atleast_1d(act), reset=False) for act in sample_lstm_act
        ]
        beats = np.array([b.size > 0 for b in beats])
        self.assertTrue(np.allclose(np.nonzero(beats)[0], [47, 79, 148, 216, 250]))
        # without resetting results are different
        beats = [
            processor.process_forward(np.atleast_1d(act), reset=False) for act in sample_lstm_act
        ]
        beats = np.array([b.size > 0 for b in beats])
        self.assertTrue(np.allclose(np.nonzero(beats)[0], [3, 79, 149, 216, 252]))
