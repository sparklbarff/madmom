...
class KeyEvaluation(EvaluationMixin):
    ...
    def tostring(self, **kwargs):
        ...
        """
        ret = f"{self.name}: " if self.name else ""
        ret += f"{self.score:3.1f}, {self.error_category}"
        return ret
...
class KeyMeanEvaluation(EvaluationMixin):
    ...
    def __init__(self, eval_objects, name=None):
        self.name = name or f"mean for {len(eval_objects):d} files"
    ...
    def tostring(self, **kwargs):
        return (
            f"{self.name}\n  Weighted: {self.weighted:.3f}  Correct: {self.correct:.3f}  Fifth: {self.fifth:.3f}  "
            f"Relative: {self.relative:.3f}  Parallel: {self.parallel:.3f}  Other: {self.other:.3f}"
        )
...
def add_parser(parser):
    ...
    # set defaults
    p.set_defaults(eval=KeyEvaluation, mean_eval=KeyMeanEvaluation, sum_eval=None, load_fn=load_key)
    # file I/O
